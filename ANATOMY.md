...
**Project:** `mind`
**Files:** 2863 | **Est. tokens:** ~6,930,479
**Generated:** 2026-06-16 23:26 UTC
...
- `results.tsv` (~1414 tok, large) — commit	val_bpb	memory_gb	status	description
- `run.log` (~181 tok, small) — === [1/3] BUILD mindc + fft_test.so + harnesses ===
...
- `harness_cref.c` (~739 tok, large) — fnv(re)^fnv(im) reduction as bench/fft/harness.c, but runs ONLY the C
...
- `RESULTS-fft-cpu-R1-Ada-2026-06-15.md` (~780 tok, large) — RESULTS — Deterministic Q16.16 N=256 FFT, CPU codegen on R1 (Windows / Ada host)
...
- `fft_q16.mind` (~2633 tok, huge) — Deterministic Q16.16 fixed-point radix-2 DIT FFT, N=256 (complex).
