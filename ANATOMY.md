...
**Project:** `mind`
**Files:** 2859 | **Est. tokens:** ~6,927,306
**Generated:** 2026-06-15 09:51 UTC
...
- `build.sh` (~159 tok, small) — Build the GPU FFT bench: MIND Q16 kernel (nvcc -O3/-O0 codegen race) + cuFFT baseline.
- `cufft_context.cu` (~1141 tok, large) — // Context-only comparison: cuFFT FP32 batched N=256 C2C forward FFT.
- `fft_q16_gpu.cu` (~1152 tok, large) — // Deterministic Q16.16 radix-2 DIT FFT, N=256, on the GPU (CUDA).
- `.gitignore` (~12 tok, tiny) — harness_gpu_O0
- `harness_gpu.cu` (~2416 tok, huge) — // GPU benchmark harness for the deterministic Q16.16 N=256 FFT.
- `RESULTS-fft-gpu-2026-06-15.md` (~607 tok, large) — RESULTS — Deterministic Q16.16 N=256 FFT on GPU (MIND vs cuFFT)
### `bench/fft/`
...
- `ir_canonical.rs` (~3721 tok, huge) — Copyright 2025 STARGA Inc.
...
- `vec.mind` (~887 tok, large) — std/vec.mind — RFC 0005 Phase 2: pure-MIND growable vector.
### `tests/autodiff/`
...
- `generics_lowering.rs` (~370 tok, medium) — Copyright 2026 STARGA Inc. Licensed under the Apache License, Version 2.0.
