...
- [~] **C2** Narrow ints (i8/i16/i32/u*) — **store/load LANDED (2026-07-21, `1372c4c`):** user-reachable
  ...
  narrow-width wrap-around ARITHMETIC — first slice landed as an additive selftest (`selftest_native_elf_narrow_add_i8`, i8 two's-complement wrap 100+100->-56 via nb_wrap_rax_w after the add); the op x width matrix now extends to `{sub,mul}xi8 + {add,mul}xi16` via the parametric `selftest_native_elf_narrow_arith(a,b,op,w)` driver (op in {add,sub,mul}, w in {8,16}; host-side width wrap `nb_narrow_wrap_w`), gated by `self_host_native_narrow_arith_batch_smoke.py` (32 cases, every driver's corpus non-vacuous >=1 genuine wrap, each wrap case proven distinct from the non-wrapped i64 value, fail-closed [-1e6,1e6] domain guard) — all byte-identical to the frozen bootstrap (RI-E1, e6509b39). A narrow-int TYPE + per-op width masking on the *general* (non-selftest) path, wider i32 wrap under a widened guard, and unsigned surface types, remain open.
