...
### Open follow-ups (2026-07-21, from the gap-map correctness batch — Opus blind-verify of `890f9c2`/`59b1318`/`99ba23a`)
- [ ] **i8/i16 REASSIGNED param HANGS (infinite loop) in the self-host native path** — `fn f(x: i8){ while.. x = x + 1 } return x` HANGS the emitted ELF (the mindc MLIR oracle is correct). Verified IDENTICAL before and after the param-mutation fix `890f9c2` — a PRE-EXISTING narrow-width-param out-of-subset gap, orthogonal to the let-env ordering change. Root-cause the narrow-width carried-var emit (width-wrap count vs emit interacting with the carry loop) before narrow params are declared supported. Related: [[reference_native_elf_toplevel_assign_gap_2026_07_21]], C2 narrow-arith.
- [ ] **Inner-scope `let` shadow is never popped from the nb_lets env** — `if x>0 { let x = 5; s = x; } return s + x` returns a WRONG value (self-host 10 vs oracle 15) because the branch-local `let x` stays bound after the block closes, so a later read of the param `x` resolves to the stale inner binding. PRE-EXISTING (pre-`890f9c2` was also wrong, 20) — the param-mutation fix only changed WHICH wrong answer, not a regression; main.mind never nests a param-shadowing `let` so byte-identity holds. This also makes `890f9c2`'s commit phrasing "an unmutated param falls through to resolve_param unchanged" slightly imprecise (a since-closed inner-scope let-shadow keeps a stale let-env binding that now wins). Fix: pop branch-local `let` bindings at block exit in the nb_lets scope (mirror lower.rs scope discipline).
