...
| Status | **Phases A, B, C Shipped** (B.2 partial — see §7) |
| Authors | STARGA Inc. |
| Created | 2026-05-22 |
| Supersedes | — |
| Superseded by | — |
...
| B ✓ | Tensor operators `@`, `.+`, `.-`, `.*`, `./`. Each desugars at a single point (`lower_expr`) to existing IR forms: `@` → `Instr::MatMul` (same as `tensor.matmul`), `.+`/`.-`/`.*`/`./` → `Instr::BinOp` (same as scalar `+`/`-`/`*`/`/`). Shape inference: `shape::matmul_mismatch` and `shape::broadcast_mismatch` diagnostics. **B.2 partial**: `.T`, `.sum`, `.mean` have SHIPPED (`phase_b2_transpose_operator`, `phase_b2_sum_reduction`, `phase_b2_mean_reduction`). Still deferred: `.max` reduction, norm shorthands, `.reshape`, and MLIR-level byte-identity with raw `matmul_rmajor_f32_v`/`dot_*_v` intrinsics — the latter two blocked on shape-dim threading from the type-checker into `lower_expr`. The two deferred cases carry explicit `#[ignore]` reasons in the test file rather than being silently absent. | IR-text byte-identity gated (RFC §7.2): `A @ B` ≡ `tensor.matmul(A, B)` via `format_ir_module`; `A .+ B` ≡ `A + B`; same for `.-`/`.*`/`./`. 20 passed / 2 ignored in `tests/rfc0012_phase_b_operators.rs` (the 2 ignored are the deferred B.2 cases above). Full suite 0 new failures (3 pre-existing mindcraft_check_cli failures unchanged). Bootstrap oracle 6/6. blas_smoke 12/12. Bench: medium_mlp -0.37% (p=0.05), large_network +3.3% (p=0.08) — all within +7% cap. |
| C ✓ | `#[deterministic]`, `#[target(...)]`, and `#[q16]` annotations. Compile-time checks: `determinism::nondeterministic_in_deterministic`, `determinism::float_in_q16_fn`, `determinism::unknown_target`. **Diagnostic-name correction:** this row originally specced the third code as `shape::target_unavailable`; the shipped code is `determinism::unknown_target` (`src/type_checker/mod.rs:6120`), which is the correct namespace — an unknown `#[target(...)]` is a determinism-contract failure, not a shape error. 21/21 Phase C tests pass (`tests/rfc0012_phase_c_annotations.rs`); 10/10 attribute-syntax tests pass (`tests/rfc0012_attribute_syntax.rs`). | `#[deterministic]` functions that operate on `q16` tensors must lower to MLIR byte-identical to the explicit `dot_q16_v` / `matmul_rmajor_q16_v` calls. `#[target(cpu)]` on an f32 function must lower identically to the current default path. |
