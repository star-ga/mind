...
- CANONICAL_DECISION = typed contiguous 8-byte cells; loads/stores via typed IR lowering to native memory ops. Un-drop IndexAssign (fail-closed until a real store path exists) — **DONE, and the real value-semantic store path (`Instr::ArrayStore` → `tensor.insert` + name rebind) is now LANDED for straight-line AND loop writes**. F2 region-exit threading is IMPLEMENTED for loops (the design-reviewed Option A+: the aggregate is threaded as a value-semantic typed `tensor<NxT>` loop iter-arg — an `IndexAssign` body arm in the `While` lowering emits `ArrayStore` + `record_loop_mut`, and the MLIR emitter types every loop-carried block-arg by its `ValueKind` and registers each exit-id so post-loop `ArrayLoad` resolves). The remaining follow-on is the **branch-region exit rebind** (the same threading for an `if`-body store), plus the `value_types` aggregate-type-invariant population for the fresh `dst` (see Step D).
- GAP = straight-line + loop `a[i]=v` work; branch / expression-position `a[i]=v` is fail-closed pending the branch-region exit rebind. No silent-wrong in any case.
- EVIDENCE = `Instr::ArrayStore` in `src/ir/mod.rs`; MLIR `tensor.insert` lowering + `loop_carry_mlir_type` block-arg typing in `src/mlir/lowering.rs`; emit + name-rebind at the fn-body `IndexAssign` statement dispatch AND the `While`-body `IndexAssign` arm (`record_loop_mut`) in `src/eval/lower.rs`; fail-closed panic for branch/expression positions. Tests: `tests/array_store_run.rs` (straight-line + loop RUN correct; branch fail-closed), `tests/mic3_array_store_roundtrip.rs` (0x2B wire fixed point), `tests/cross_substrate_identity/array-store-loop/` (loop-carry byte-identity canary avx2==neon, no per-iteration copy).
- **`SILENT_STORE_DROP` = IMPOSSIBLE_OR_FAIL_CLOSED** is the required end state for i64/f32/f64 alike — **now enforced in every case** (straight-line + loop store correctly; branch fails closed).
