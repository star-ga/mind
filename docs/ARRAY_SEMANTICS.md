...
- CANONICAL_DECISION = typed contiguous 8-byte cells; loads/stores via typed IR lowering to native memory ops. Un-drop IndexAssign (fail-closed until a real store path exists) — **DONE, and the real value-semantic store path (`Instr::ArrayStore` → `tensor.insert` + name rebind) is now LANDED for straight-line AND loop writes**. F2 region-exit threading is IMPLEMENTED for loops (Fable-approved Option A+: the aggregate is threaded as a value-semantic typed `tensor<NxT>` loop iter-arg — an `IndexAssign` body arm in the `While` lowering emits `ArrayStore` + `record_loop_mut`, and the MLIR emitter types every loop-carried block-arg by its `ValueKind` and registers each exit-id so post-loop `ArrayLoad` resolves). The remaining follow-on is the **branch-region exit rebind** (the same threading for an `if`-body store), plus the `value_types` aggregate-type-invariant population for the fresh `dst` (see Step D).
