...
- CANONICAL_DECISION = typed contiguous 8-byte cells; loads/stores via typed IR lowering to native memory ops. Un-drop IndexAssign — **DONE and COMPLETE: the real value-semantic store path (`Instr::ArrayStore` → `tensor.insert` + name rebind) is LANDED for straight-line, LOOP, AND BRANCH writes** (the design-reviewed Option A+: the aggregate is a value-semantic typed `tensor<NxT>` threaded through both the `While` loop-carried machinery — `IndexAssign` body arm + `record_loop_mut`, typed header/body/`^while_after` iter-args — and the `Instr::If` merge machinery — then/else `IndexAssign` arms + `record_then_write`/`record_else_write`, typed `^if_after` merge block-arg via `mlir_type`; the not-taken edge carries the pre-if tensor). The only remaining follow-on is `value_types` aggregate-type-invariant population for the fresh `dst` (see Step D, an internal-invariant hardening; the runtime behaviour is already correct + wedge-safe).
- GAP = NONE for a fixed-aggregate `a[i]=v` in statement position (straight-line / loop / branch all correct). A non-bare-Ident receiver (e.g. `f()[i]=v`) or a pure expression-position store still fails-closed by design (no silent-wrong).
- EVIDENCE = `Instr::ArrayStore` in `src/ir/mod.rs`; MLIR `tensor.insert` lowering + `loop_carry_mlir_type` / `mlir_type` block-arg typing in `src/mlir/lowering.rs`; emit + rebind at the fn-body `IndexAssign` statement dispatch, the `While`-body arm (`record_loop_mut`), AND the then/else `Instr::If` branch arms (`record_then_write`/`record_else_write`) in `src/eval/lower.rs`. Tests: `tests/array_store_run.rs` (straight-line + loop + branch all RUN correct), `tests/mic3_array_store_roundtrip.rs` (0x2B wire fixed point), `tests/cross_substrate_identity/array-store-loop/` (loop-carry byte-identity canary avx2==neon, no per-iteration copy).
- **`SILENT_STORE_DROP` = ELIMINATED** for a fixed-aggregate `a[i]=v` — every statement position (straight-line / loop / branch) stores correctly with the byte-identity wedge intact; genuinely-unsupported receiver shapes fail-closed, never silent-wrong.
