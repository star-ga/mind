#!/usr/bin/env python3
"""RI-B2-S11 (#108) — native-ELF PACKED-f32 SIMD (SSE, 128-bit) STRICT-FP DOT-PRODUCT.

The float SIMD rung: proves the pure-MIND native-ELF backend emits VECTOR packed-f32
SIMD (movups / mulps / addps) whose f32 result is byte-identical to the SAME MLIR
canary dot-f32-v the scalar S8 rung hit (len 4093, seed 0xDEADBEEF). f32 add is NOT
associative, so byte-identity rests on reproducing ref_dot_f32_strict's 8-lane fold
BIT-FOR-BIT with a 2x-XMM (acc_lo lanes 0..3, acc_hi lanes 4..7) packed accumulate,
a pinned left-to-right horizontal fold, and a scalar tail. Deterministic strict-FP
SIMD, zero MLIR/LLVM.

`selftest_native_elf_simd_dot_f32(n)` emits a runnable x86-64 ET_EXEC that:
  ...
  (2) computes the STRICT-FP dot with the EXACTLY-pinned fold
      ...
      via packed mulps/addps (per-lane unfused, no FMA) into 2 XMM accumulators,
...
Usage:
  MINDC_SO=/path/to.so python3 self_host_native_simd_dot_f32_smoke.py
"""
import ctypes
import hashlib
import os
...
# dot_f32_v_reproducibility_gate. IDENTICAL to the scalar S8 rung target.
...
def mind_simd_dot_f32_elf(lib, n: int) -> bytes:
    fn = lib.selftest_native_elf_simd_dot_f32
...
def main() -> int:
    ...
    if not os.path.exists(so):
        ...
        return 0
    lib = ctypes.CDLL(so)
    if not hasattr(lib, "selftest_native_elf_simd_dot_f32"):
        print("FAIL  selftest_native_elf_simd_dot_f32: symbol absent (RI-B2-S11 not built)")
        return 1

    with tempfile.TemporaryDirectory() as td:
        tmp = pathlib.Path(td)
        elf = mind_simd_dot_f32_elf(lib, LENGTH)
        if not (len(elf) > 120 and elf[:4] == b"\x7fELF"):
            print(f"  FAIL  simd_dot_f32(n={LENGTH}): not a runnable ELF (len={len(elf)})")
            return 1
        ...
        if ok:
            print("ALL PASS  native-ELF PACKED-f32 SIMD (SSE, 128-bit) strict-FP dot is "
                  "BYTE-IDENTICAL to the MLIR canary dot-f32-v — 2x-XMM 8-lane packed "
                  "accumulate (movups/mulps/addps, unfused) + pinned left-to-right fold + "
                  "scalar tail, zero MLIR/LLVM. Native-ELF f32 SIMD == scalar == MLIR "
                  "(RI-B2-S11 #108, float SIMD rung).")
            return 0
        ...
        print("FAIL  native-ELF f32 SIMD dot hash != canary dot-f32-v — the lane mapping "
              "or the fold order differs; report the native f32/bits/hash above and STOP "
              "(do NOT guess; float byte-identity is fragile).")
        return 1
