#!/usr/bin/env python3
"""RI-B2-S6 (#108) — native-ELF scalar GEMV Q16.16 (matrix x vector), byte-identity rung.

`selftest_native_elf_gemv_q16(m, n)` emits a runnable x86-64 ET_EXEC that:
  (1) generates W[0..m*n] then x[0..n] via the EXACT Q16.16 LCG the cross-substrate
      canary gemv-q16 uses (seed 0xDEADBEEF; state = state*1664525 + 1013904223;
      element next_q16 = ((state>>16) as u32 as i32) >> 12), W row-major then x,
  (2) for each row r computes y[r] = Sum_c (sext(W[r,c]) * sext(x[c])) >> 16 with a
      FULL i64 accumulator (movsxd DWORD loads, per-product Q16 rescale), narrows
      ONCE to i32 (movsxd) — the MLIR canary's `acc as i32`,
...
cross-substrate reference for `gemv-q16` (canonical_hash_i32s of the y vector).
Zero MLIR/LLVM in the native path.

Usage:
  MINDC_SO=/path/to.so python3 self_host_native_gemv_q16_smoke.py
"""
import ctypes
import hashlib
import os
...
# Committed cross-substrate reference: canary `gemv-q16` (256x256, seed 0xDEADBEEF)
# = canonical_hash_i32s(y) = sha256 of each i32 y[r].to_le_bytes() row-major
# (tests/cross_substrate_identity.rs gemv_q16_reproducibility_gate).
CANARY = "dfdf890874472ee369da524955995889c39bc6da770e4e2b1d0d69315e17611a"
ROWS = 256
COLS = 256


def mind_gemv_q16_elf(lib, m: int, n: int) -> bytes:
    fn = lib.selftest_native_elf_gemv_q16
