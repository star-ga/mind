    ...
    let mut out: Vec<(String, DepKind)> = manifest
        ...
        .filter_map(|(name, spec)| match spec {
            DependencySpec::Path { path, .. } => {
                Some((name.clone(), DepKind::Path(path.clone())))
            }
            DependencySpec::Git { git, rev, tag, branch, .. } => Some((
                name.clone(),
                DepKind::Git {
                    url: git.clone(),
                    rev: rev.clone(),
...
fn load_lockfile(path: &Path) -> Result<MindLock> {
    ...
    let lock: MindLock = toml::from_str(&text)
...
fn write_lockfile(path: &Path, lock: &MindLock) -> Result<(), DepError> {
    ...
    fs::write(path, full)
        .map_err(|e| DepError::Io(format!("cannot write {}: {e}", path.display())))
}
...
fn check_lockfile_staleness(
    ...
    let lock_names: BTreeSet<&str> =
        lock.dependencies.iter().map(|e| e.name.as_str()).collect();
    let manifest_names: BTreeSet<&str> =
        relevant.iter().map(|(n, _)| n.as_str()).collect();
    ...
    if !missing.is_empty() || !extra.is_empty() {
        ...
        if !missing.is_empty() {
            msg.push_str(&format!("\n  Missing from Mind.lock: {}", missing.join(", ")));
        }
        if !extra.is_empty() {
            msg.push_str(&format!(
...
fn resolve_dep_to_lock_entry(
    ...
    match kind {
        ...
        DepKind::Git { url, rev, tag, branch } => {
...
fn resolve_git_dep(
    ...
    let tmp = tempfile::tempdir()
        .map_err(|e| DepError::Io(format!("temp dir: {e}")))?;
    let clone_dir = tmp.path().join("repo");

    let ok = shallow_clone(url, &fetch_ref, &clone_dir).is_ok();
    if !ok {
        full_clone_and_checkout(url, &fetch_ref, &clone_dir)
            .map_err(|e| DepError::FetchFailed(format!(
                "cannot fetch git dep '{}' ({}@{}): {e}", name, url, fetch_ref
            )))?;
    }

    let full_sha = git_rev_parse(&clone_dir, "HEAD")
        .map_err(|e| DepError::FetchFailed(format!(
            "rev-parse HEAD for '{}': {e}", name
        )))?;
    ...
    let cache_dir = git_cache_dir(&provisional_entry)
        .map_err(|e| DepError::Io(format!("cache dir: {e}")))?;
...
fn git_cache_dir(entry: &LockEntry) -> Result<PathBuf> {
    ...
    let rev = if entry.rev.is_empty() { "unknown" } else { &entry.rev };
