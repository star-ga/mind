...
/// int8 tier row block. Tuned to 1024: a rigorous pinned-core c3 sweep at KC=512/NC=768
/// found MC is the DOMINANT blocking knob — the old MC=64 re-streamed the packed-B panel
/// 16× (badly memory-bound). MC sweep (int8 VNNI GMAC/s): 64→56, 256→109, 512→123,
/// **1024→135 ≈ 2.0× single-core OpenBLAS f32** (c3-confirmed 134.8 = 2.02× WITH the
/// heap-scratch; byte-exact at every step: the c3 VNNI gate
/// PASSED all 14 K-straddling shapes; the int8 canary 917d353b is unchanged — MC only
/// regroups the associative reduction). MC=1024 is safe only because the C-scratch + packed
/// panels are now HEAP-allocated (@malloc, see emit_mm_i8_blocked) — ~6 MiB at MC=1024/NC=768
/// would overflow constrained caller stacks as an `alloca`. Robust for any blocking + caller.
pub const I8_MC: usize = 1024;
