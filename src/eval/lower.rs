...
/// (the `scalar_math` bench, the keystone) where they are never consulted. The
/// design already guarantees an UNUSED prelude is output-invariant, so skipping
/// it is byte-identical. A false-negative (skip-when-needed) is impossible: a
/// `false` result is only produced for fully-understood literal/arith shapes,
/// and such a tree contains no node that can read the prelude tables.
#[cfg(feature = "std-surface")]
fn may_reference_enum_prelude(n: &ast::Node) -> bool {
    use ast::Node as N;
    match n {
        N::Lit(Literal::Int(_), _)
        | N::Lit(Literal::Float(_), _)
        | N::Lit(Literal::Str(_), _) => false,
...
pub fn lower_to_ir(module: &ast::Module) -> IRModule {
    ...
    #[cfg(feature = "std-surface")]
    {
        ...
        // could actually reference them. A pure-scalar module (scalar_math, the
        // keystone) provably never consults these side-tables, and an unused
