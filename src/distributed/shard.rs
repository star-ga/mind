...
impl ShardSpec {
    pub fn replicated(world: u32) -> Self {
        Self { layout: ShardLayout::Replicated, world, deterministic: true }
    }

    pub fn split(axis: u8, world: u32) -> Self {
        Self { layout: ShardLayout::Split { axis }, world, deterministic: true }
    }

    pub fn split2d(row_axis: u8, col_axis: u8, world: u32) -> Self {
        Self { layout: ShardLayout::Split2D { row_axis, col_axis }, world, deterministic: true }
    }
    ...
    pub fn local_shape(&self, global: &[u32]) -> Result<Vec<u32>, ShardingError> {
        match self.layout {
            ...
            ShardLayout::Split { axis } => {
                ...
                let a = axis as usize;
                if a >= local.len() {
                    return Err(ShardingError::AxisOutOfRange { axis, ndim: local.len() });
                }
                if local[a] % self.world != 0 {
                    return Err(ShardingError::NotEvenlyDivisible { dim: local[a], world: self.world });
                }
                local[a] /= self.world;
                Ok(local)
            }
            ShardLayout::Split2D { row_axis, col_axis } => {
                ...
                let col_world = if row_world == 0 { 0 } else { self.world / row_world };
                ...
                if (row_axis as usize) >= local.len() || (col_axis as usize) >= local.len() {
                    return Err(ShardingError::AxisOutOfRange { axis: row_axis.max(col_axis), ndim: local.len() });
                }
                if local[row_axis as usize] % row_world != 0 {
                    return Err(ShardingError::NotEvenlyDivisible { dim: local[row_axis as usize], world: row_world });
                }
                if local[col_axis as usize] % col_world != 0 {
                    return Err(ShardingError::NotEvenlyDivisible { dim: local[col_axis as usize], world: col_world });
                }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ShardingError::AxisOutOfRange { axis, ndim } => {
                write!(f, "shard axis {} out of range for tensor with {} dims", axis, ndim)
            }
            ShardingError::NotEvenlyDivisible { dim, world } => {
                write!(f, "dimension {} not evenly divisible by world size {}", dim, world)
            }
            ShardingError::Not2DDivisible(world) => {
                write!(f, "world size {} is not a perfect square for 2D split", world)
            }
        }
    }
}
...
mod tests {
    ...
    fn split_rejects_non_divisible() {
        let spec = ShardSpec::split(0, 3);
        assert!(matches!(spec.local_shape(&[25, 768]), Err(ShardingError::NotEvenlyDivisible { .. })));
    }

    #[test]
    fn split_rejects_axis_out_of_range() {
        let spec = ShardSpec::split(5, 3);
        assert!(matches!(spec.local_shape(&[24, 768]), Err(ShardingError::AxisOutOfRange { .. })));
    }
    ...
    fn shard_layout_renders() {
        ...
        assert_eq!(
            ShardLayout::Split2D { row_axis: 0, col_axis: 1 }.to_string(),
            "split2d(row=0, col=1)"
        );
    }
