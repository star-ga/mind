...
fn decode_instr<R: Read>(
    ...
    match op {
        ...
        OP_CONST_DENSE_TENSOR => {
            ...
            let shape = read_shape(r, strings, limit)?;
            let n = read_count(r, limit)?;
            let mut data = Vec::with_capacity(bounded_cap(n, limit));
            for _ in 0..n {
                data.push(read_u64_le(r)?);
            }
            Ok(Instr::ConstDenseTensor {
                dst,
                dtype,
                shape,
                data,
...
pub fn parse_mic3(data: &[u8]) -> Result<IRModule, Mic3Error> {
    ...
    let mut module = IRModule {
        ...
        // The boxed-enum set is a lowering-only side-table; like
        ...
        // Struct-variant field-name table — lowering-only, never serialised.
        #[cfg(feature = "std-surface")]
