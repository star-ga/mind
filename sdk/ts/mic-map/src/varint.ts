 ...
 */
export function uleb128Write(out: number[], value: bigint): void {
 ...
 */
export function uleb128Read(buf: Uint8Array, pos: number): [bigint, number] {
  let result = 0n;
  let shift = 0n;
  let i = pos;
 ...
 */
export function zigzagDecode(encoded: bigint): bigint {
 ...
 */
export function sleb128Write(out: number[], value: bigint): void {
 ...
 */
export function sleb128Read(buf: Uint8Array, pos: number): [bigint, number] {
  const [encoded, newPos] = uleb128Read(buf, pos);
  return [zigzagDecode(encoded), newPos];
}

/** Calculate encoded size in ULEB128 bytes. */
export function uleb128Size(value: bigint): number {
  let v = value;
  let size = 0;
  do {
    size++;
