...
import { type Mic2Module, type GraphValue, type Opcode, opcodeToken } from "./types.js";
 ...
 */
export function emit(module: Mic2Module): string {
  const parts: string[] = [];
...
function emitValue(v: GraphValue): string {
  switch (v.kind) {
...
function emitNode(op: Opcode, inputs: readonly number[]): string {
...
function emitOpcode(op: Opcode): string {
