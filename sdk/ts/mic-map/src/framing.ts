 ...
 */
export function framePayload(payload: string | Uint8Array): Uint8Array {
  ...
  try {
    while (true) {
      ...
      while (buf.length < HEADER_SIZE) {
        ...
        if (done) {
          if (buf.length === 0) return; // clean end
      ...
      if (frameLen === 0) throw new FrameDecodeError("zero-length frame");
      if (frameLen > MAX_FRAME_SIZE) throw new FrameTooLarge(frameLen, MAX_FRAME_SIZE);
      ...
      while (buf.length < totalNeeded) {
        const { done, value } = await reader.read();
        if (done) throw new FrameDecodeError("stream ended mid-frame");
        buf = concat(buf, value);
      }
...
function concat(a: Uint8Array<ArrayBuffer>, b: Uint8Array<ArrayBufferLike>): Uint8Array<ArrayBuffer> {
