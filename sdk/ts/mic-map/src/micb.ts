...
import {
  type Mic2Module, type GraphValue, type Opcode, type TensorType,
  dtypeToByte, dtypeFromByte, opcodeToByte,
} from "./types.js";
import {
 ...
 */
export function encodeBinary(module: Mic2Module): Uint8Array {
  const out: number[] = [];
 ...
 */
export function decodeBinary(bytes: Uint8Array): Mic2Module {
...
class MicbEncoder {
  ...
  constructor(private readonly module: Mic2Module) {}

  encode(out: number[]): void {
    this.buildStringTable();

    // Magic + version
    for (const b of MICB_MAGIC) out.push(b);
  ...
  private buildStringTable(): void {
    ...
    for (const sym of this.module.symbols) this.intern(sym);
    for (const t of this.module.types) {
      for (const dim of t.shape) this.intern(dim);
    }
  ...
  private encodeStringTable(out: number[]): void {
  ...
  private encodeSymbolTable(out: number[]): void {
  ...
  private encodeTypeTable(out: number[]): void {
  ...
  private encodeValueTable(out: number[]): void {
  ...
  private encodeValue(out: number[], v: GraphValue): void {
    if (v.kind === "arg" || v.kind === "param") {
  ...
  private encodeOpcode(out: number[], op: Opcode): void {
    out.push(opcodeToByte(op));

    switch (op.kind) {
      case "softmax": sleb128Write(out, op.axis); break;
      case "transpose":
        uleb128Write(out, BigInt(op.perm.length));
        for (const p of op.perm) sleb128Write(out, p);
        break;
      case "sum": case "mean": case "max":
        uleb128Write(out, BigInt(op.axes.length));
        for (const a of op.axes) sleb128Write(out, a);
        break;
      case "concat": sleb128Write(out, op.axis); break;
      case "split":
        sleb128Write(out, op.axis);
        uleb128Write(out, BigInt(op.n));
        break;
      case "gather": sleb128Write(out, op.axis); break;
      case "custom":
...
class MicbDecoder {
  ...
  private checkMagic(): void {
    if (this.buf.length < 4) throw new MicbError("truncated: missing magic");
    for (let i = 0; i < 4; i++) {
  ...
  private checkVersion(): void {
    if (this.pos >= this.buf.length) throw new MicbError("truncated: missing version");
  ...
  private decodeStringTable(): string[] {
    const n = this.readUleb();
    const result: string[] = [];
    const decoder = new TextDecoder();
    for (let i = 0; i < n; i++) {
      const len = this.readUleb();
      if (this.pos + len > this.buf.length) throw new MicbError("truncated string table");
  ...
  private decodeSymbolTable(): string[] {
    const n = this.readUleb();
    const result: string[] = [];
    for (let i = 0; i < n; i++) {
      const idx = this.readUleb();
      if (idx >= this.strings.length) throw new MicbError(`symbol string index ${idx} out of bounds`);
      result.push(this.strings[idx]!);
    }
    return result;
  }

  private decodeTypeTable(): TensorType[] {
    const n = this.readUleb();
    const result: TensorType[] = [];
    for (let i = 0; i < n; i++) {
      if (this.pos >= this.buf.length) throw new MicbError("truncated type table");
      const dtypeByte = this.buf[this.pos++]!;
      const dtype = dtypeFromByte(dtypeByte);
      if (!dtype) throw new MicbError(`unknown dtype byte: ${dtypeByte}`);

      const rank = this.readUleb();
      const shape: string[] = [];
      for (let j = 0; j < rank; j++) {
        const idx = this.readUleb();
        if (idx >= this.strings.length) throw new MicbError(`type dim string index ${idx} out of bounds`);
  ...
  private decodeValueTable(nTypes: number): GraphValue[] {
    const n = this.readUleb();
  ...
  private decodeValue(currentId: number, nTypes: number): GraphValue {
    if (this.pos >= this.buf.length) throw new MicbError("truncated value table");
    const tag = this.buf[this.pos++]!;

    if (tag === 0 || tag === 1) {
      const nameIdx = this.readUleb();
      const typeIdx = this.readUleb();
      if (nameIdx >= this.strings.length) throw new MicbError(`value name index ${nameIdx} out of bounds`);
      if (typeIdx >= nTypes) throw new MicbError(`value type index ${typeIdx} out of bounds`);
    ...
    if (tag === 2) {
      ...
      for (let i = 0; i < nInputs; i++) {
        ...
        if (inp >= currentId) throw new MicbError(`forward reference: input ${inp} >= current id ${currentId}`);
        inputs.push(inp);
      }
  ...
  private decodeOpcode(): Opcode {
    if (this.pos >= this.buf.length) throw new MicbError("truncated opcode");
    ...
    switch (b) {
      ...
      case 11: {
        const n = this.readUleb();
        const perm: bigint[] = [];
        for (let i = 0; i < n; i++) perm.push(this.readSleb());
      ...
      case 13: {
        const n = this.readUleb();
        const axes: bigint[] = [];
        for (let i = 0; i < n; i++) axes.push(this.readSleb());
      ...
      case 255: {
        const idx = this.readUleb();
        if (idx >= this.strings.length) throw new MicbError(`custom opcode name index ${idx} out of bounds`);
