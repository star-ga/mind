...
function buildLine(prefix, fields) {
...
export function decode(line) {
    ...
    if (trimmed.length === 0)
        throw new MapDecodeError("empty line");
    const prefix = trimmed[0];
    if (prefix === "@") {
        const { head, rest } = splitFirstWord(trimmed.slice(1));
        if (!head)
            throw new MapDecodeError("missing op after @");
    ...
    if (prefix === "=") {
        ...
        if (kw === "err") {
            ...
            if (!code)
                throw new MapDecodeError("missing error code after =err");
    ...
    if (prefix === "!") {
        const { head, rest } = splitFirstWord(trimmed.slice(1));
        if (!head)
            throw new MapDecodeError("missing event name after !");
...
function splitFirstWord(s) {
    ...
    if (spaceIdx === -1)
        return { head: trimmed || undefined, rest: "" };
...
function parseFields(s) {
    ...
    while (rest.length > 0) {
        ...
        if (eqIdx <= 0)
            throw new MapDecodeError(`expected key=value, got: '${rest}'`);
 ...
 */
function parseValue(s, inList = false) {
    if (s.length === 0)
        throw new MapDecodeError("expected value, got empty string");
    // List: [...]
    if (s[0] === "[")
        return parseList(s);
    // Quoted string: "..."
...
function parseList(s) {
    ...
    while (i < s.length) {
        ...
        if (items.length > 0) {
            if (s[i] !== ",")
                throw new MapDecodeError(`expected ',' in list, got '${s[i] ?? "EOF"}'`);
            i++;
            while (i < s.length && /\s/.test(s[i]))
                i++;
        }
...
function parseQuotedString(s) {
    ...
    while (i < s.length) {
        ...
        if (ch === "\\") {
            i++;
            if (i >= s.length)
                throw new MapDecodeError("unterminated escape sequence");
            const esc = s[i];
            if (esc === '"')
                result += '"';
            else if (esc === "\\")
                ...
                throw new MapDecodeError(`unknown escape: \\${esc}`);
        }
        else {
            result += ch;
        }
...
function parseHex(s, inList = false) {
    const stopRe = inList ? /[\s,\]]/ : /[\s]/;
    const end = s.search(stopRe);
    const tok = end === -1 ? s : s.slice(0, end);
    const hex = tok.slice(2); // strip 0x
    if (hex.length % 2 !== 0)
        throw new MapDecodeError(`odd-length hex: '${tok}'`);
