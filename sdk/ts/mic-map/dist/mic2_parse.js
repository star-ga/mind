...
import { parseDType, opcodeArity, } from "./types.js";
...
class Mic2Parser {
    ...
    parse() {
        ...
        while (this.lineNum < this.lines.length) {
            ...
            if (first === "S")
                this.parseSymbol(tokens);
            else if (first === "O")
                this.parseOutput(tokens);
            else if (first === "a")
                this.parseArg(tokens);
            else if (first === "p")
                this.parseParam(tokens);
    ...
    parseSymbol(tokens) {
        if (tokens.length !== 2)
            throw this.err("invalid symbol: expected 'S <name>'");
        this.symbols.push(tokens[1]);
    }
    parseType(first, rest) {
        const idxStr = first.slice(1);
        const idx = parseInt(idxStr, 10);
        if (isNaN(idx))
            throw this.err(`invalid type index: ${idxStr}`);
        if (idx !== this.types.length) {
            throw this.err(`type indices must be sequential: expected T${this.types.length}, got T${idx}`);
        }
        if (rest.length === 0)
            throw this.err("type requires dtype");
        const dtype = parseDType(rest[0]);
        if (!dtype)
            throw this.err(`unknown dtype: ${rest[0]}`);
    ...
    parseArg(tokens) {
        if (tokens.length !== 3)
            throw this.err("invalid arg: expected 'a <name> T<typeidx>'");
        const typeIdx = this.parseTypeRef(tokens[2]);
        this.addValue({ kind: "arg", name: tokens[1], typeIdx });
    }
    parseParam(tokens) {
        if (tokens.length !== 3)
            throw this.err("invalid param: expected 'p <name> T<typeidx>'");
        const typeIdx = this.parseTypeRef(tokens[2]);
        this.addValue({ kind: "param", name: tokens[1], typeIdx });
    }
    parseNode(opcodeTok, rest) {
        const inputs = [];
        const params = [];
        ...
        for (const inp of inputs) {
            if (inp >= this.values.length) {
                throw this.err(`forward reference: input ${inp} not yet defined (current max: ${this.values.length - 1})`);
            }
        }
        const op = parseOpcode(opcodeTok, params);
        if (!op)
            throw this.err(`unknown opcode: ${opcodeTok}`);
    ...
    parseOutput(tokens) {
        if (tokens.length !== 2)
            throw this.err("invalid output: expected 'O <value_id>'");
        const id = parseInt(tokens[1], 10);
        if (isNaN(id))
            throw this.err(`invalid output id: ${tokens[1]}`);
    ...
    parseTypeRef(tok) {
        if (!tok.startsWith("T"))
            throw this.err(`expected type ref T<idx>, got '${tok}'`);
        const idx = parseInt(tok.slice(1), 10);
        if (isNaN(idx))
            throw this.err(`invalid type ref: ${tok}`);
        if (idx >= this.types.length) {
...
function parseOpcode(tok, params) {
    const firstParam = params[0];
    const parseBigInt = (s, def) => {
        if (s === undefined)
            return def;
        try {
            return BigInt(s);
    ...
    const parseBigIntArr = (ps) => ps.flatMap(s => { try {
        return [BigInt(s)];
    }
    catch {
        return [];
...
//# sourceMappingURL=mic2_parse.js.map