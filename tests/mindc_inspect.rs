...
fn inspect_reports_summary_and_evidence() {
    ...
    assert!(s.contains("determinism:      deterministic"), "missing determinism:\n{s}");
    assert!(s.contains("--- canonical IR ---"), "missing IR body:\n{s}");

    // --json surfaces the same fields, charset-safe.
    let j = mindc().arg("inspect").arg(&art).arg("--json").output().unwrap();
...
fn inspect_diff_detects_divergence_and_identity() {
    ...
    let same = mindc().arg("inspect").arg(&a).arg("--diff").arg(&a).output().unwrap();
    ...
    let diff = mindc().arg("inspect").arg(&a).arg("--diff").arg(&b).output().unwrap();
    assert_eq!(diff.status.code(), Some(1), "differing artifacts must exit 1");
    let s = String::from_utf8_lossy(&diff.stdout);
    assert!(s.contains("identical:        NO"), "missing NO verdict:\n{s}");
...
fn inspect_malformed_artifact_exits_1_not_panic() {
    ...
    assert_eq!(out.status.code(), Some(1), "malformed artifact must fail closed, not panic");

    // --json parse-error path still emits a well-formed object to stdout (like verify).
    let j = mindc().arg("inspect").arg(&bad).arg("--json").output().unwrap();
    ...
    let self_diff = mindc().arg("inspect").arg(&bad).arg("--diff").arg(&bad).output().unwrap();
    assert_eq!(self_diff.status.code(), Some(1), "byte-identical garbage must not exit 0");
}
